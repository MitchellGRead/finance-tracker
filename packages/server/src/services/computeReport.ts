...
export function computeReport(
  ...
  const effectiveAmount = (item: LineItemRow) =>
    item.isCredit ? -item.amount : item.amount;

  const isPersonal = (item: LineItemRow) => item.splitRatio === 1.0;
  ...
  // Their share is amount * splitRatio; the rest is distributed evenly among all other users.

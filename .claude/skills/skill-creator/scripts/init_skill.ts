...
async function initSkill(skillName: string, path: string): Promise<string | null> {
     ...
     * @returns Path to created skill directory, or null if error
     */
    ...
    try {
        ...
        console.log(`❌ Error creating SKILL.md: ${error}`);
        return null;
    }
    ...
    try {
        ...
        await writeFile(exampleScript, scriptContent, 'utf-8');
        ...
        await writeFile(exampleReference, referenceContent, 'utf-8');
        ...
        await writeFile(exampleAsset, EXAMPLE_ASSET, 'utf-8');
        console.log('✅ Created assets/example_asset.txt');
    } catch (error) {
        console.log(`❌ Error creating resource directories: ${error}`);
        return null;
    }
